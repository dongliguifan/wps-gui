    ...
    if (hasMap === true) {
        ...
        if (!this.ui_.inputMaps[mapId]) {
            ...
            this.ui_.inputMaps[mapId].vector = new ol.layer.Vector({
                source: this.ui_.inputMaps[mapId].source, style: function (feature) {
...
wps.ui.load = function (ui, evt, nodes) {
    ...
    if (nodes !== null) {
        ...
        for (var i = 0, ii = ui.nodes.length; i < ii; ++i) {
            ...
            var node = ui.nodes[i];
            if (node.type === "process") {
                var process = ui.client_.getProcess(ui.defaultServer_, node._info.identifier.value, {
...
wps.ui.prototype.createCanvas = function () {
    this.outer = d3.select("#chart").append("svg:svg").attr("width", this.spaceWidth).attr("height", this.spaceHeight).attr("pointer-events", "all").style("cursor", "crosshair");
    this.vis = this.outer.append('svg:g').on("dblclick.zoom", null).append('svg:g').on("mousedown", $.proxy(wps.ui.canvasMouseDown, null, this)).on("mouseup", $.proxy(wps.ui.canvasMouseUp, null, this)).on("mousemove", $.proxy(wps.ui.canvasMouseMove, null, this));
    var outer_background = this.vis.append('svg:rect').attr('width', this.spaceWidth).attr('height', this.spaceHeight).attr('fill', '#fff');
...
wps.ui.prototype.createLinkPaths = function () {
    var me = this;
    var link = this.vis.selectAll(".link").data(this.nodes.filter(function (d) {
    ...
    if (ui.mouseMode === 2 && ui.mousedownNode) {
        ...
        if (!existingLink) {
            var checkMatch = function (input, output) {
                ...
                if (input._info.boundingBoxData && output._info.boundingBoxOutput) {
                    match = true;
                }
                return match;
            };
...
wps.ui.prototype.createInputLink = function (node, d, text) {
    if (d.inputs > 0) {
        var me = this;
        text.attr("x", 8);
        node.append("rect").attr("class", "port port_input").attr("rx", 3).attr("ry", 3).attr("x", -5).attr("width", 10).attr("height", 10).attr("y", 10).on("mouseup", $.proxy(wps.ui.portMouseUp, null, this, 1, 0)).on("mousedown", $.proxy(wps.ui.portMouseDown, null, this, 1, 0)).on("mouseout", function (d) {
...
wps.ui.prototype.updateNode = function (d, ui) {
    ...
    if (d.dirty && d.w && d.h) {
        ...
        d._ports.enter().append("rect").attr("class", "port port_output").attr("rx", 3).attr("ry", 3).attr("width", 10).attr("height", 10).on("mouseout", function (d) {
            var port = d3.select(this);
            port.classed("port_hovered", false);
        }).on("mouseover", function (d, i) {
            var port = d3.select(this);
...
wps.ui.prototype.createProcessRect = function (node) {
    var mainRect = node.append("rect").attr("class", function (d) {
        return "node node_" + d.type + (!d.complete ? " node_incomplete" : "") + (d.required ? " node_required" : "");
    }).attr("rx", 6).attr("ry", 6).on("mouseup", $.proxy(wps.ui.nodeMouseUp, null, this)).on("mousedown", $.proxy(wps.ui.nodeMouseDown, null, this));
};
